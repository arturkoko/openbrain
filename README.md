...
- Private + business email, phone, and address fields
...
- Synced via **Syncthing** — changes appear in Obsidian instantly
...
| AI | OpenRouter → Gemini 2.5 Flash (function calling) |
...
**Prerequisites:** A Linux server or VM with Docker + Docker Compose. A domain with Cloudflare is optional (for remote access).
...
Default login is set via `.env` (`DEFAULT_ADMIN_USER` / `DEFAULT_ADMIN_PASSWORD`).
...
Cloud AI assistants are convenient. But they may train on your data, can shut down or change pricing, and every sync adds another company with access to your information.

OpenBrain keeps everything on your hardware. The AI model runs in the cloud (OpenRouter), but **no personal data leaves your server** — only your questions do.
...
> *"Dictate my daily note: Had a strategy session with Gerald about the KI project, decided to go with fine-tuning instead of RAG. Ran 8km. Good day overall."*

AI transcribes audio → structures it as an Obsidian daily note → writes to `reviews/daily/YYYY-MM-DD.md` → synced to your devices instantly.

**Personal CRM**
> *"Who haven't I talked to in a while that I should reconnect with?"*

AI queries your VIP contacts sorted by `last_contact` and flags anyone overdue.

**Household Intelligence**
> *"What's the paint color for the living room?"*

AI queries `household_items` and answers instantly — no digging through notes.

**Calendar via Chat**
> *"Block Thursday afternoon for deep work, 2–5pm."*

AI creates a CalDAV event that appears in your iOS Calendar within seconds.
